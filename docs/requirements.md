...
## Core Concept: チェック → 自動トリアージによる人間確認の削減

> **このコンセプトはプロジェクト全体の設計方針であり、将来の機能追加時も必ず遵守すること。**

### 基本方針

FoCheck / BoCheck はルールベースの機械チェックであり、失敗したチェックの内容は **必ずエージェント（FoAgent / BoAgent）がトリアージする**。エージェントが自動解決できる問題（SSI 登録・BIC 修正・説明付与など）は人間の確認なしに完結し、本当に判断が必要なケースのみ人間にエスカレーションする。

```
FoCheck 実行
  → 失敗なし → FoValidated（人間確認ゼロ）
  → 失敗あり → FoAgent トリアージ（失敗チェック名 + エラーメッセージを入力として渡す）
                → 自動解決 → FoValidated（人間確認ゼロ）
                → HITL 必要 → オペレーター承認（1 件のみ）
                → 判断困難 → FoUser にエスカレーション
```

### 実装上の不変ルール

1. **チェック失敗結果の引き渡しは必須**: FoTriage / BoTriage を開始する際は、失敗したチェック名とエラーメッセージを `error_message` に格納してエージェントに渡す。空文字での呼び出しは禁止（バリデーションでエラーになる）。
2. **UI でのトリアージ開始制限**: FoCheck/BoCheck の失敗がない状態では、対応するトリアージ開始ボタンを非活性にする。
3. **エラーメッセージのフォーマット**: `[{rule_name}] {message}` 形式で、失敗チェックを改行区切りで列挙する。追加コンテキストは末尾に付記する。
4. **自動トリガー（将来対応）**: `fo_check_trigger=auto` の場合、FoCheck 実行直後に FoAgent が自動起動される（手動トリガーは UI からの明示的な実行）。
