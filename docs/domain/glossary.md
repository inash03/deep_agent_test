...
| **Counterparty Search** | Finding a counterparty during trade creation by **name-prefix** or **partial-LEI** match. The result set is the candidate counterparties an operator chooses from; selecting one binds its LEI (and name, for display) to the trade. |
